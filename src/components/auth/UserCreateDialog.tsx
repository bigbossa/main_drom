...
export const UserCreateDialog = ({
  ...
  const handleFormSubmit = (data: UserFormData) => {
   ...
   const fullAddress = `บ้านเลขที่ ${data.houseNumber} หมู่ที่ ${data.village} ${streetPart} ตำบล ${data.subDistrict} อำเภอ ${data.district} จังหวัด ${data.province} `;
                            ...
                            <SelectItem key={room.id} value={room.id}>
                              <div className="flex items-center justify-between w-full">
                                ...
                                <div className="flex items-center gap-2 ml-2">
                                  <Badge
                                    variant={room.current_occupants === 0 ? "secondary" : "outline"}
                                  >
